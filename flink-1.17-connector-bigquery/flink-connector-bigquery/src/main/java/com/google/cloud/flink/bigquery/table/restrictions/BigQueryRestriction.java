...
public class BigQueryRestriction {
    ...
    public static Optional<String> convert(Expression flinkExpression) {
        ...
        if (op == null) {
            // Need to check first if op is null, otherwise we will get an NPE trying to call
            // ordinal on
            // null. If unknown, we try to apply the filter on the Flink side.
            return Optional.empty();
        }
        switch (op) {
            case IS_NULL:
            ...
            default:
                // Defensive check if there exists something in FILTERS we don't explicitly handle
                return Optional.empty();
        }
    }
